...
void icmp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
  ...
  {
    errorf("icmp_input: data length is too short");
    return;
  }

  if (cksum16((uint16_t *)data, len, 0) != 0)
  {
    errorf("icmp_input: checksum error");
    return;
  }
  ...
  case ICMP_TYPE_ECHO:
    ...
    icmp_output(ICMP_TYPE_ECHOREPLY, hdr->code, hdr->values, data + sizeof(struct icmp_hdr), len - sizeof(struct icmp_hdr), iface->unicast, src);
    break;
  default:
    /* ignore */
    break;
