...
arp_request(struct net_iface *iface, ip_addr_t tpa)
{
    ...
    request.hdr.hrd = ARP_HRD_ETHER;
    request.hdr.pro = ARP_PRO_IP;
    request.hdr.hln = ETHER_ADDR_LEN;
    request.hdr.pln = IP_ADDR_LEN;
    request.hdr.op = ARP_OP_REQUEST;
...
arp_reply(struct net_iface *iface, const uint8_t *tha, ip_addr_t tpa, const uint8_t *dst)
{
    ...
    reply.hdr.hrd = ARP_HRD_ETHER;
    reply.hdr.pro = ARP_PRO_IP;
    reply.hdr.hln = ETHER_ADDR_LEN;
    reply.hdr.pln = IP_ADDR_LEN;
    reply.hdr.op = ARP_OP_REPLY;
...
arp_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    if (msg->hdr.hrd != ARP_HRD_ETHER) {
        errorf("hrd: 0x%04x", ntoh16(msg->hdr.hrd));
        return;
    }
    if (msg->hdr.pro != ARP_PRO_IP) {
        errorf("pro: 0x%04x", ntoh16(msg->hdr.pro));
        return;
    }
...
arp_init(void)
{
    return net_protocol_register(NET_PROTOCOL_TYPE_ARP, arp_input);
}
